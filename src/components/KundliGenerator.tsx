...
export const KundliGenerator: React.FC<KundliGeneratorProps> = ({
  ...
  // Filtered Cities for Autocomplete using World Cities Database
  const filteredCities = useMemo(() => {
    return searchWorldCities(citySearchQuery, 25);
  }, [citySearchQuery]);
  ...
  const handleSelectCity = (c: CityPreset | WorldCity) => {
    ...
    setBirthDetails((prev) => ({
      ...prev,
      city: `${c.name}, ${regionPart}${c.country}`,
      latitude: c.lat,
      longitude: c.lng,
    ...
    <div className="space-y-6">
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex-1 min-w-0">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold font-vedic text-amber-100 tracking-tight">
              {hasGenerated ? 'Janam Kundali & Natal Dossier' : 'Kundali Generator — Enter Birth Details'}
            </h1>
          ...
          <form onSubmit={handleGenerate} className="space-y-5">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-stone-700 mb-1.5 flex items-center justify-between">
                  ...
                  {birthDetails.gender === 'female' && (
                    <span className="text-[10px] font-bold text-pink-700 bg-pink-50 border border-pink-200 px-1.5 py-0.5 rounded">
                      ♀ Female
                    </span>
                  )}
                </label>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <button
                  type="button"
                  id="btn-choose-from-map-link"
                  onClick={() => setIsMapModalOpen(true)}
                  className="text-xs font-semibold text-amber-800 hover:text-amber-900 flex items-center gap-1.5 transition-colors cursor-pointer group"
                >
                  <Globe className="w-3.5 h-3.5 text-amber-800 group-hover:rotate-12 transition-transform" />
                  <span className="underline decoration-amber-300 underline-offset-2">Choose from Map</span>
                </button>
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2">
                ...
                <div className="relative flex-1">
                  <input
                    type="text"
                    value={citySearchQuery || birthDetails.city}
                    onChange={(e) => {
                      setCitySearchQuery(e.target.value);
                      setIsCityDropdownOpen(true);
                    }}
                    onFocus={() => setIsCityDropdownOpen(true)}
                    placeholder="Search city e.g. New Delhi, Mumbai, New York, London, Tokyo, Dubai..."
                    className="w-full h-11 pl-10 pr-10 rounded-xl border border-stone-300 focus:border-amber-700 focus:ring-1 focus:ring-amber-700 text-sm bg-stone-50/50 shadow-2xs transition-colors"
                  />
                  ...
                  {/* City Dropdown Menu */}
                  {isCityDropdownOpen && (
                    <div className="absolute left-0 right-0 mt-1 max-h-60 overflow-y-auto bg-white border border-stone-200 rounded-xl shadow-xl z-40 divide-y divide-stone-100 text-xs">
                      <div className="p-2 bg-stone-50 text-[11px] text-stone-500 font-medium flex justify-between items-center sticky top-0 backdrop-blur-xs">
                        <span>Select from verified world cities:</span>
                        <button
                          type="button"
                      ...
                      {filteredCities.map((c) => (
                        <button
                          ...
                          className="w-full px-3.5 py-2.5 text-left hover:bg-amber-50 flex items-center justify-between transition-colors group cursor-pointer"
                        >
                          <div>
      ...
      {isMapModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-5 overflow-y-auto">
          <div className="bg-white rounded-2xl border border-stone-200 shadow-2xl max-w-4xl w-full max-h-[92vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-150">
